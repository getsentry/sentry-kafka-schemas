...
def run(target_folder="python/sentry_kafka_schemas/schema_types/"):
    ...
    for topic_name in sentry_kafka_schemas.sentry_kafka_schemas._list_topics():
        ...
        for schema_meta in topic_meta['schemas']:
            ...
            module_index[schema_tmp_typename] = schema_tmp_module_name, schema_data['schema'].get('title', 'Main')
            subprocess.check_call(
                ["jsonschema-gentypes", "--json-schema", schema_data['schema_filepath'], "--python", os.path.join(target_folder, f"{schema_tmp_module_name}.py")]
            )
