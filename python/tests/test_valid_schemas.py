...
def test_schemas_valid(topic: str, version: int) -> None:
    ...
    def _validate_title(obj: Any) -> None:
        ...
        if ("properties" in obj or "additionalProperites" in obj or "patternProperties" in obj or "required" in obj) and obj.get("type") != "object":
            # Impose restriction so that types will be good:
            # https://github.com/sbrunner/jsonschema-gentypes/issues/469
            raise AssertionError(f"type=object needs to be specified explicitly on all schemas, if properties are defined")
